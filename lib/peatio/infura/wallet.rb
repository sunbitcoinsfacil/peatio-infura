  ...
  module Ethereum
    class Wallet < Peatio::Wallet::Abstract
      ...
      def get_transaction_count(address)
        client.json_rpc(:eth_getTransactionCount, [normalize_address(address), "latest"])
      rescue Ethereum::Client::Error => e
        raise Peatio::Wallet::ClientError, e
      end

      def sign_transaction(data)
        client.json_rpc(:eth_signTransaction, [ data ], conn_type="sign")
      rescue Ethereum::Client::Error => e
        raise Peatio::Wallet::ClientError, e
      end
      ...
      def create_eth_transaction!(transaction, options = {})
        ...
        permit_transaction(@wallet.fetch(:address), @wallet.fetch(:secret))
        signed_hash = sign_transaction({
          ...
          nonce: get_transaction_count(@wallet.fetch(:address))
        })
      ...
      def create_erc20_transaction!(transaction, options = {})
        ...
        signed_hash = sign_transaction({
          ...
          data:     data,
          gas:      '0x' + options.fetch(:gas_limit).to_i.to_s(16),
          gasPrice: '0x' + '0x' + options.fetch(:gas_price).to_i.to_s(16),
          nonce: get_transaction_count(@wallet.fetch(:address))
        })
      ...
      def permit_transaction(address, password)
        client.json_rpc(:personal_unlockAccount, [normalize_address(address), password, 5], conn_type="sign")
      rescue Ethereum::Client::Error => e
        raise Peatio::Wallet::ClientError, e
      end
      ...
      def client
        ...
        sign_uri = @wallet.fetch(:sign_uri) { raise Peatio::Wallet::MissingSettingError, :uri }
